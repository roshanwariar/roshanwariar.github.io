...
But using Sequential Monte Carlo as an probabilistic inference-time scaling algorithm is not my idea or new. The paper Rollout Roulette by Puri, et al. [4] looks at using lanaguage models as a proposal mechanism again and then having a process reward model to grade multiple different chains-of-thought traces with scores which are normalized and used as weights over which a distribution can be described and SMC can operate. Personally, the idea of using a PRM isn't very elegant and I wanted to see how far we could push the reasoning performance of the base model without a critic model or external tools.<br> 

So to setup my own SMC algorithm, I...
