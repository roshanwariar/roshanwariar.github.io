...
This is a pretty standard formulation with \$p_{prop}\$ representing the proposal distribution (the low-temperature sampling of the language model). However, I think the real insight is the choice for the target distribution: \$\pi(\mathbf{x})\$ which is proportional to the "sharpened" version of the proposal distribution \$p(\mathbf{x})^\alpha\$. They call this _power sampling_, and their intuition is that this method upweights next tokens who's future tokens will be of higher likelihood. 
